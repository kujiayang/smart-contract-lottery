...
contract FundSubscription is Script {
    ...
    function fundSubscription(address vrfCoordinator, uint64 subId, address link, uint256 deployerKey) public {
        ...
        if (block.chainid == 31337) { // if is on local network
            vm.startBroadcast(deployerKey);
            VRFCoordinatorV2Mock(vrfCoordinator).fundSubscription(subId, FUND_AMOUNT);
            vm.stopBroadcast();
        } else { // else on sepolia
            vm.startBroadcast(deployerKey);
...
contract AddConsumer is Script {
    ...
    function addConsumerUsingConfig(address raffle) public {
        HelperConfig helperConfig = new HelperConfig();
        // try the function in UI to find out the function called
        (, , address vrfCoordinator, , uint64 subId, , , uint256 deployerKey) = helperConfig.activeNetworkConfig();
        addConsumer(raffle, vrfCoordinator, subId, deployerKey);

    }

    function run() external { // $ forge install ChainAccelOrg/foundry-devops --no-commit
